...
= Plane-fitting Algorithm (DRAFT) <setup>
 ...
 Let $MMM = (V,F)$, where $V : = {x_k : k in [1,N_V]} subset RR^3$ and $F : = {(bold(v_i),bold(v_j),bold(v_k)) : 1 <= i,j,k <= N_F}$, be a triangle mesh in $RR^3 $ that bounds a solid region $Omega $.
...
//   +  is “transverse” in the sense of being *approximately perpendicular to the object’s long axis*.
 ...
 For an oriented triangle mesh with face $f  in F$, compute these using signed tetrahedra about a reference point $bold(r) in RR^(3)$ (often $bold(r) = 0 $ or the mesh’s bounding-box center):
 ...
 Now, define the centered covariance matrix: 
...
//  In this case, $u $ is the direction of maximum spread of the point cloud—usually the limb’s “length” direction.
 ...
 Recall that a unit vector $bold(hat(n))$ is tangent to a plane $Pi $ iff its normal $bold(m) $ satisfies #numbered_eq($  bold(m) dot bold(hat(n)) = 0   $)<eq-11>
 ...
 #numbered_eq($  (max_(bold(m) in RR^(3))(bold(m) dot bold(u)) "s.t"  ((norm(bold(v)) = 1) and (bold(m) dot bold(hat(n)) = 0)))  $)<eq-12>
 ...
 #numbered_eq($  bold(m) = frac(op("proj")_(bold(hat(n))^(perp))(bold(u)), norm(op("proj")_(bold(hat(n))^(perp)) (bold(u)))) = frac(bold(u -(u dot hat(n))hat(n)), bold(norm(u -(u dot hat(n))hat(n))))  $)<eq-13>
...
//   -  Since $m approx u $, the plane is approximately perpendicular to the long axis (transverse slice).
 ...
 Finally, the plane through the centroid with normal $bold(m) $ is 
 #numbered_eq($  Pi = {bold(x) in RR^(3): bold(m) dot (bold(x - c))= 0 }  $)

 Equivalently, in $(a x + b x + c x + d = 0) $ form: $ (a,b,c)= bold(m) $ 
 #numbered_eq($ d = - (bold(m dot c))  $)

 /* \hrule */

 Do note, if $bold(u) $ is nearly parallel to $n $, then 
 #numbered_eq($  bold(u -(u dot n)n) approx 0  $) 
 ...
 For each mesh vertex $bold(x) in S$, where $bold(x)=(x_i,y_i,z_i)$, compute:
 #numbered_eq($ s_i A $)

 FINISH ALGO DETAILS
 
 THEN ADD MAIN WORKFLOW
