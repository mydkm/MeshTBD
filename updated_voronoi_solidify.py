...
DEFAULT_SOLIDIFY_THICKNESS = 2.5
DEFAULT_SOLIDIFY_OFFSET = 1.0
...
def ensure_blender_object_mode() -> None:
...
def recalculate_blender_normals(obj) -> None:
    select_only_blender_object(obj)
    ensure_blender_object_mode()
...
def apply_blender_solidify_modifier(
    obj,
    *,
    thickness: float,
    offset: float = DEFAULT_SOLIDIFY_OFFSET,
    ...
    if thickness <= 0:
        raise ValueError("Solidify thickness must be positive.")
    if not -1.0 <= offset <= 1.0:
        raise ValueError("Solidify offset should be between -1.0 and 1.0.")

    select_only_blender_object(obj)
    ensure_blender_object_mode()
    bpy.ops.object.transform_apply(location=False, rotation=False, scale=True)
    recalculate_blender_normals(obj)

    solid = obj.modifiers.new(name="Solidify", type="SOLIDIFY")
    solid.thickness = float(thickness)
    solid.offset = float(offset)

    if hasattr(solid, "use_even_offset"):
        solid.use_even_offset = use_even_offset
    if hasattr(solid, "use_quality_normals"):
        solid.use_quality_normals = use_quality_normals
    if hasattr(solid, "use_rim"):
        solid.use_rim = use_rim
    if hasattr(solid, "use_rim_only"):
        solid.use_rim_only = use_rim_only

    bpy.ops.object.modifier_apply(modifier=solid.name)
    recalculate_blender_normals(obj)

    print(
        "Applied Blender Solidify modifier "
        f"(thickness={thickness:.4f}, offset={offset:.4f}, "
        f"even_offset={use_even_offset}, rim={use_rim}, rim_only={use_rim_only})."
    )
...
def main() -> int:
    ...
    ap.add_argument(
        ...
        help=(
            "Blender Solidify thickness in mesh units for the final mesh "
            f"(default: {DEFAULT_SOLIDIFY_THICKNESS})."
        ),
    )
    ap.add_argument(
        "--solidify-offset",
        type=float,
        default=DEFAULT_SOLIDIFY_OFFSET,
        help=(
            "Blender Solidify offset in [-1, 1]. 1.0 pushes outward, 0.0 centers, "
            f"-1.0 pushes inward (default: {DEFAULT_SOLIDIFY_OFFSET})."
        ),
    )
    ...
    if args.solidify_thickness <= 0:
        ap.error("--solidify-thickness must be positive.")
    if not -1.0 <= args.solidify_offset <= 1.0:
        ap.error("--solidify-offset must be between -1.0 and 1.0.")
    ...
    stage_banner("Blender Solidify and Final Export")
    ...
    apply_blender_solidify_modifier(
        final_obj,
        thickness=args.solidify_thickness,
        offset=args.solidify_offset,
