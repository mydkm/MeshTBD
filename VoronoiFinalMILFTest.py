...
def main() -> int:
    ...
    # Optional MILF sanity check for the initial PyVista surface
    md_input = from_pyvista(surf)
    print("MILF PyVista -> MeshData:", md_input.summary())
    ...
    # ----------------------------
    # MILF handoff: PyMeshLab -> MeshData -> PyVista
    # ----------------------------
    csurface = ms.current_mesh()
    md_voronoi = from_pymeshlab(csurface)
    print("MILF PyMeshLab -> MeshData:", md_voronoi.summary())

    cmesh = to_pyvista(md_voronoi)
    print("MILF MeshData -> PyVista completed!")

    # Optional round-trip sanity check
    md_voronoi_roundtrip = from_pyvista(cmesh)
    print("MILF PyVista round-trip:", md_voronoi_roundtrip.summary())
    ...
    elif "Colors" in cmesh.point_data:
        rgba = np.asarray(cmesh.point_data["Colors"])
    else:
        raise SystemExit("No vertex color array found after PyMeshLab -> PyVista conversion.")

    if rgba.ndim == 1:
        rgba = rgba.reshape(n_pts, -1)
    ...
    if "Colors" in cmesh.point_data:
        cmesh.point_data.pop("Colors")
    ...
    red_faces = red_mesh.faces.reshape(-1, 4)[:, 1:].astype(np.int32)
    ...
    # ----------------------------
    # MILF handoff: PyMeshLab -> MeshData -> Blender
    # ----------------------------
    smooth_mesh = ms.current_mesh()
    md_smooth = from_pymeshlab(smooth_mesh)
    print("MILF PyMeshLab -> MeshData:", md_smooth.summary())

    obj = to_bpy_object_linked(md_smooth, bpy=bpy, name="pymlMesh")
    bpy.context.view_layer.objects.active = obj
    obj.select_set(True)
    print("MILF MeshData -> Blender completed!")
