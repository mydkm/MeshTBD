...
# class PlaneCalcResult:
#     signed_volume: float
#     centroid: np.ndarray
#     closest_point: np.ndarray
#     closest_cell_id: int
#     closest_dist2: float
#     arrow_direction: np.ndarray
#     principal_axes: np.ndarray
#     plane_normal: np.ndarray
#     plane_offset: float
#     plane_signed_distance: np.ndarray
...
#     if edges.n_cells > 0:
#         print(
#             f"[WARN] Mesh appears to have boundary edges (count={edges.n_cells}). "
#             "Volume/centroid from signed tetrahedra may be invalid."
#         )
...
# def extract_triangle_faces(mesh: pv.PolyData) -> np.ndarray:
#     faces_raw = np.asarray(mesh.faces, dtype=np.int64).reshape(-1, 4)
#     if faces_raw.size == 0:
#         raise ValueError("Mesh has no faces.")
#     if not np.all(faces_raw[:, 0] == 3):
#         raise ValueError("Mesh is not purely triangles.")
#     return faces_raw[:, 1:4]
...
#     pts = np.asarray(mesh.points, dtype=np.float64)
#     faces = extract_triangle_faces(mesh)
...
#     if plane_mode == "normal":
#         return normalize(arrow_direction)

#     if plane_mode == "longitudinal":
#         u = u0
#         u_perp = u - np.dot(u, arrow_direction) * arrow_direction
#         if np.linalg.norm(u_perp) < 1e-6:
#             u = u1
#             u_perp = u - np.dot(u, arrow_direction) * arrow_direction
#         if np.linalg.norm(u_perp) < 1e-6:
#             u = pick_nonparallel_axis(arrow_direction)
#             u_perp = u - np.dot(u, arrow_direction) * arrow_direction
#         return normalize(np.cross(arrow_direction, normalize(u_perp)))

#     if plane_mode == "transverse":
#         u = u0
#         m = u - np.dot(u, arrow_direction) * arrow_direction
#         if np.linalg.norm(m) < 1e-6:
#             u = u1
#             m = u - np.dot(u, arrow_direction) * arrow_direction
#         if np.linalg.norm(m) < 1e-6:
...
#     raise ValueError(f"Unsupported plane_mode: {plane_mode}")
...
#     signed_distance = points @ plane_normal + plane_offset
#     band_mask = np.abs(signed_distance) <= ep
#     band_idx = np.where(band_mask)[0]

#     if band_idx.size == 0:
#         band_idx = np.argsort(np.abs(signed_distance))[:1]
#         band_mask = np.zeros_like(signed_distance, dtype=bool)
#         band_mask[band_idx] = True
#         print(
#             f"[WARN] No vertices satisfied abs(Ax+By+Cz+D) <= ep (ep={ep}). "
#             "Falling back to the single closest-to-plane vertex as band."
#         )

#     return signed_distance, band_mask, band_idx


# def compute_plane_pipeline(
#     mesh: pv.PolyData,
#     ref_mode: str = "mesh_center",
#     plane_mode: str = "transverse",
#     ep: float | None = None,
# ) -> PlaneCalcResult:
#     mesh = ensure_polydata(mesh).triangulate()
#     pts = np.asarray(mesh.points, dtype=np.float64)
...
#     signed_volume, centroid = compute_volume_and_centroid_signed_tets(mesh, ref=ref)
...
#     arrow_direction = normalize(closest_point - centroid)
#     axes = pca_axes(pts, center=centroid)
#     plane_normal = compute_plane_normal(arrow_direction, axes, plane_mode)
#     plane_offset = -float(np.dot(plane_normal, centroid))

#     ep_use = ep if ep is not None else 0.01 * (diag if diag > 0 else 1.0)
#     plane_signed_distance, band_mask, band_idx = select_plane_band(pts, plane_normal, plane_offset, ep_use)
#     band_points = pts[band_idx]

#     return PlaneCalcResult(
...
#         plane_signed_distance=plane_signed_distance,
#         band_mask=band_mask,
#         band_idx=band_idx,
...
#     distance_mode: str,
#     diffusion_k: int = 60,
#     diffusion_t: float = 0.01,
#     geodesic_backend: str = "auto",
...
#         d_to_band = compute_euclidean_distance_to_band(pts, band_points)
#         return d_to_band, "dist_norm (euclidean to band)"

#     if distance_mode == "diffusion":
#         print("=== Diffusion distance ===")
#         print(f"Building diffusion embedding with k={diffusion_k}, t={diffusion_t} ...")
#         emb, evals_use = diffusion_embedding(pts, faces, k=int(diffusion_k), t=float(diffusion_t))
#         print(
#             f"Computed {emb.shape[1]} diffusion coords. "
#             f"Eigenvalue range: [{evals_use.min():.6g}, {evals_use.max():.6g}]"
#         )
#         d_to_band = compute_diffusion_distance_to_band(emb, band_idx)
#         return d_to_band, "dist_norm (diffusion to band)"

#     if distance_mode == "geodesic":
#         print("=== Geodesic distance (graph shortest path) ===")
#         print(f"Backend: {geodesic_backend}")
#         d_to_band = geodesic_distance_to_band(pts, faces, band_idx, backend=geodesic_backend)
...
#         return d_to_band, "dist_norm (geodesic to band)"

#     raise ValueError(f"Unsupported distance_mode: {distance_mode}")


# # ----------------------------
# # Mesh scalar attachment
# # ----------------------------

# def attach_plane_scalars(mesh: pv.PolyData, result: PlaneCalcResult, d_to_band: np.ndarray) -> None:
#     dist_norm = normalize_01(d_to_band)

#     side = np.where(result.plane_signed_distance < 0.0, -1.0, 1.0)
#     dist_signed = d_to_band * side
#     dist_signed_norm = dist_norm * side

#     mesh.point_data["plane_signed_dist"] = result.plane_signed_distance
#     mesh.point_data["in_band"] = result.band_mask.astype(np.uint8)
...
#     ap.add_argument("--show-band", action="store_true")
#     ap.add_argument("--show-edges", action="store_true")
#     ap.add_argument("--no-show", action="store_true", help="Compute/export scalars without opening a PyVista window.")
#     return ap
...
#     D = result.plane_offset
#     u0 = normalize(result.principal_axes[:, 0])

#     print("=== Plane + Geometry ===")
#     print(f"Signed volume V = {result.signed_volume:.6g}   (abs(V)={abs(result.signed_volume):.6g})")
#     print(f"Centroid c = {result.centroid}")
#     print(
#         f"Closest point p = {result.closest_point}  "
#         f"(cell_id={result.closest_cell_id}, dist={np.sqrt(result.closest_dist2):.6g})"
#     )
#     print(f"Arrow direction n = {result.arrow_direction}")
#     print(f"Principal axis u0 = {u0}")
...
#     # Distance field
#     # ----------------------------
#     d_to_band, title = compute_distance_to_band(
#         mesh=mesh,
...
#     attach_plane_scalars(mesh, result, d_to_band)

#     print("=== Coloring ===")
#     print(f"ep = {result.ep:.6g}   (#band_vertices={result.band_idx.size})")
#     print(f"dist_to_band: min={float(np.min(d_to_band)):.6g}, max={float(np.max(d_to_band)):.6g}")
...
#         q = np.asarray(mesh.point_data[scalar_to_export], dtype=np.float64).reshape(-1)
#         faces_tri = extract_triangle_faces(mesh)
#         save_ply_with_vertex_quality(args.export_ply, np.asarray(mesh.points), faces_tri, q)
#         print(f"[OK] Exported PLY with vertex quality '{scalar_to_export}' to: {args.export_ply}")
...
#         plotter.add_mesh(pv.Sphere(radius=r, center=result.centroid))
#         plotter.add_mesh(pv.Sphere(radius=r, center=result.closest_point))

#         plotter.add_mesh(pv.Arrow(start=result.centroid, direction=result.arrow_direction, scale=0.25 * (diag if diag > 0 else 1.0)))
#         plotter.add_mesh(pv.Arrow(start=result.centroid, direction=u0, scale=0.20 * (diag if diag > 0 else 1.0)))
#         plotter.add_mesh(pv.Arrow(start=result.centroid, direction=result.plane_normal, scale=0.18 * (diag if diag > 0 else 1.0)))

#         if args.show_band:
#             band_poly = pv.PolyData(result.band_points)
#             plotter.add_mesh(band_poly, render_points_as_spheres=True, point_size=8)
